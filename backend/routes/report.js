...
router.get('/dashboard', protect, async (req, res) => {
  try {
    ...
    // Pending payments (from sale agreements)
    ...
    const pendingPaymentsTotal = activeAgreements.reduce((sum, agreement) => {
      return sum + (agreement.totalAmount - agreement.downPayment);
    }, 0);

    const pendingCustomersCount = activeAgreements.length;
