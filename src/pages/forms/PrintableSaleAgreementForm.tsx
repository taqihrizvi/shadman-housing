...
export default function PrintableSaleAgreementForm({ data, onClose }: PrintableSaleAgreementFormProps) {
  ...
  const handleDownloadPDF = async () => {
    ...
    const fileName = `Sale-Agreement-${data.agreementNumber}-Plot-${plotNumber}.pdf`;
  ...
  const remainingBalance = data.totalAmount - data.downPayment;
    ...
    <div className="bg-gray-50 max-h-screen overflow-auto">
      ...
      <div className="print:hidden sticky top-0 z-50 bg-white border-b shadow-sm">
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          <h2 className="text-lg font-semibold">Sales Agreement Form - {data.agreementNumber}</h2>
      ...
      <div ref={contentRef} className="bg-white max-w-full">
          <div className="p-6">
            ...
            <div className="text-center mb-8 border-b-2 border-gray-800 pb-4">
              <h1 className="text-3xl font-bold mb-2">SALES AGREEMENT</h1>
              <p className="text-base">Shadman Greens Housing - Phase I & Phase II</p>
              <p className="text-sm mt-2">Agreement No: {data.agreementNumber}</p>
              <p className="text-sm">Agreement Date: {formatDate(data.agreementDate)}</p>
            </div>
            ...
            <div className="relative">
              ...
              <div className="ml-0 mr-44">
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div className="flex items-baseline gap-3">
                    <span className="font-semibold whitespace-nowrap w-40">Contact Number:</span>
                    <div className="border-b-2 border-black px-3 py-1 flex-1">{data.customer.phone}</div>
                  </div>

                  <div className="flex items-baseline gap-3">
                    <span className="font-semibold whitespace-nowrap w-40">Address:</span>
                ...
                <div className="mb-6 text-sm leading-relaxed text-justify">
                  <p>
                    This agreement is made between Party A (Seller - Shadman Greens Housing) and Party B (Buyer - {data.customer.name}), 
                    where both parties have mutually agreed without any coercion or duress, with their free consent. 
                    Party B agrees to purchase a plot of land with the following details through cash and easy installments 
                    as per the terms and conditions mentioned herein.
                  </p>
                </div>

                {/* Plot Details */}
                <h2 className="text-xl font-bold text-center mb-4">PLOT DETAILS & PAYMENT INFORMATION</h2>
                  ...
                  <table className="w-full">
                    <tbody>
                      ...
                      <tr className="border-b-2 border-black">
                        ...
                        <td className="p-3 font-bold">{formatCurrency(data.totalAmount)}</td>
                      </tr>
                      <tr className="border-b-2 border-black">
                        <td className="border-r-2 border-black p-3 font-bold">Down Payment</td>
                        <td className="p-3 font-bold">{formatCurrency(data.downPayment)}</td>
                      </tr>
                      <tr className="border-b-2 border-black">
                        <td className="border-r-2 border-black p-3 font-semibold">Total Installments</td>
                        <td className="p-3">{data.installmentMonths > 0 ? `${data.installmentMonths} Months` : 'Full Payment'}</td>
                      </tr>
                      <tr className="border-b-2 border-black">
                        <td className="border-r-2 border-black p-3 font-semibold">Monthly Installment</td>
                        <td className="p-3">{data.monthlyAmount > 0 ? formatCurrency(data.monthlyAmount) : 'N/A'}</td>
                      </tr>
                      <tr className="border-b-2 border-black">
                        <td className="border-r-2 border-black p-3 font-semibold">Remaining Balance</td>
                        <td className="p-3">{formatCurrency(remainingBalance)}</td>
                      </tr>
                      <tr>
                        <td className="border-r-2 border-black p-3 font-semibold">Possession Date</td>
                        <td className="p-3">{data.possessionDate ? formatDate(data.possessionDate) : 'After Full Payment'}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            <div className="mb-4 text-sm leading-relaxed text-justify break-inside-avoid">
              <p>
                I, {data.customer.name} s/o {data.customer.fatherName}, solemnly declare that the said plot is located 
                in Shadman Greens Housing {formatEnum(data.plot.project)} and I accept all terms and conditions of this agreement. 
                I understand that this is a legally binding contract and I will fulfill all my obligations as mentioned herein.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-8 mb-4 break-inside-avoid">
              <div className="text-center">
                {data.status === 'APPROVED' && data.createdBy?.signature ? (
                  <div className="flex flex-col items-center">
                    <div className="w-full border-b-2 border-black h-16 mb-2 flex items-center justify-center">
                      <img 
                        src={`http://localhost:5000${data.createdBy.signature}`} 
                        alt="Admin Signature" 
                        className="max-h-14 max-w-full object-contain"
                        onLoad={() => console.log('✅ Signature loaded:', `http://localhost:5000${data.createdBy.signature}`)}
                        onError={(e) => {
                          console.error('❌ Signature failed to load:', `http://localhost:5000${data.createdBy.signature}`);
                          console.error('Error details:', e);
                        }}
                      />
                    </div>
                    <p className="font-bold text-sm">Signature of Party A</p>
                    <p className="text-xs mt-1">(Seller - {data.createdBy.name})</p>
                  </div>
                ) : (
                  <>
                    <div className="border-b-2 border-black h-16 mb-2"></div>
                    <p className="font-bold text-sm">Signature of Party A</p>
                    <p className="text-xs mt-1">(Seller{data.createdBy?.name ? ` - ${data.createdBy.name}` : ''})</p>
                  </>
                )}
              </div>
              <div className="text-center">
                ...
                <p className="text-xs mt-1">(Buyer - {data.customer.name})</p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="border-b-2 border-black h-8 px-2 flex items-center">
